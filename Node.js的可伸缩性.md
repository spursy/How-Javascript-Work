...
在生产环境下，我通常使用进程检测器(PM2)。PM2提供许多监测Node.js应用性能的命令，使用PM2处理各种任务都非常简单。例如：只要在命令参数后面添加-i，就可以对应用使用集群功能：
...
万物总是有利有弊。对Node.js应用做负载均衡时，必然会失去单进程所具有的许多特性。就像其它开发语言都要面对如进程间共享数据这样的进程安全问题。在我们这里，就是多个子进程共享数据的问题。
...
尽管如此，也不要认为使用独立的服务做缓存是集群的弊端。因为使用独立的缓存服务是使用分解策略增强Node.js应用的可伸缩性的一种方式。即便Node.js应用部署在单核服务器上，也推荐开发者使用这种方式。

除了数据缓存，状态通信也会存在问题。因为在集群应用中，某个子服务对外部请求做状态标记后，并不能确保当该外部节点再次发送请求时，仍旧可以分配给与上次相同的子服务上。因此在子服务上创建程序的状态标记并不是好选择。
...
如果开发者不想使用修改代码实现在共享数据库中存储请求信息的方案，这样将不会有代码上的侵略性但是会很低效。这里可以考虑**粘滞负载均衡**方案。由于许多负载均衡器都支持这种策略，因此可以很轻松完成对应的代码。原理其实很简单：如果用户的会话信息保存在子进程A上，主进程会存储会话与进程的索引信息。

![](https://user-gold-cdn.xitu.io/2018/4/22/162eca9208e162a7?w=2000&h=1125&f=png&s=169554)

当相同用户再次请求服务时，程序会在主进程的索引表上查询存储该用户会话信息的进程，然后将请求任务分配给该子进程。这里我们并没有对请求实现负载均衡，但是如果在一些受限的场景下，这也是不错的选择。
